...
The Zvvm family of Integrated Matrix extensions (Zvvmm, Zvvfmm, Zvvmtls) provides the following subextensions:
...
* The _input matrices_ A and B are stored in vector register groups with element width determined by the instruction:
  ...
  The K dimension of the multiply equals λ for non-packing instructions, 2λ for double-packing, and 4λ for quad-packing.
...
The `lambda[2:0]` field is a 3-bit WARL read/write field in the `vtype` CSR that holds the dynamic _selected lambda_ — the effective K dimension used for registers backing the output matrix C.
...
.Editorial Note
====
The `altfmt_A` and `altfmt_B` bits are currently assigned to `vtype[9]`
...
For `vwmmacc.vv` and `vqwmmacc.vv`, `vm=0` encodes `vfwimmacc.vv` and
`vfqwimmacc.vv` — integer-input, floating-point-accumulate, E8M0-microscaled
...
stem:[C_{m,n} \leftarrow C_{m,n} + \sum_{k=0}^{λ W - 1} A_{m,k} \times B_{k,n}]
...
stem:[C_{m,n} \leftarrow round(C_{m,n} + (round(S)))],

where the roundings are performed with the rounding mode from `frm`.
The rounding of partial sum S _before_ it is accumulated to the running value of stem:[C_{m,n}] is optional.
...
Microscaling via `v0.scale` is supported on all floating-point
multiply-accumulate instructions (`vfmmacc.vv`, `vfwmmacc.vv`, `vfqwmmacc.vv`)
and on the integer-input FP-accumulate instructions `vfwimmacc.vv` (MXINT8→FP16/BF16)
and `vfqwimmacc.vv` (MXINT4→FP16/BF16, MXINT8→FP32).
...
// SEW=8 (Int4 → OFP8), SEW=32, SEW=64 are reserved
if EEW_C == 8  then return Illegal_Instruction();
if EEW_C == 32 then return Illegal_Instruction();
if EEW_C == 64 then return Illegal_Instruction();
...
// BS=16 LMUL restriction: W × LMUL ≤ SEW
if vtype[bs] == 0b1 & (2 * LMUL > EEW_C) then return Illegal_Instruction();
// Microscaling legality: SEW × λ ≥ 16
if EEW_C * lambda < 16 then return Illegal_Instruction();

// altfmt_A=1 and altfmt_B=1 are reserved: MXINT inputs are signed
...
foreach (j from 0 to (N - 1)) {
  foreach (i from 0 to (M - 1)) {
    ...
    foreach (s from 0 to (S - 1)) {
      // Read paired 16-bit scale element from v0
      let pair_A : bits(16) = read_single_element(16, i * R + s, zvreg(0));
      let pair_B : bits(16) = read_single_element(16, j * R + s, zvreg(0));

      let sA : bits(EEW_C) = e8m0_to_fp(pair_A[7..0],  EEW_C, fmt_C);
      let sB : bits(EEW_C) = e8m0_to_fp(pair_B[15..8], EEW_C, fmt_C);
      let blk_scale : bits(EEW_C) = fp_mul(sA, sB, EEW_C, fmt_C, rm);
      ...
      // Exact signed integer dot product within block
      let k_lo : int = s * block_size;
      let k_hi : int = min((s + 1) * block_size, K_eff) - 1;
      var int_sum : int = 0;
      foreach (k from k_lo to k_hi) {
        let a_flat : int = mat_A_idx(i, k, K_eff, LMUL, lambda, epr_A);
        let b_flat : int = mat_B_idx(k, j, K_eff, LMUL, lambda, epr_A);
        let a_val  : int = signed(read_single_element(EEW_A, a_flat, vs1));
        let b_val  : int = signed(read_single_element(EEW_A, b_flat, vs2));
        int_sum = int_sum + a_val * b_val
      };
      // Convert exact integer sum to FP, apply block scale, accumulate
      let fp_sum : bits(EEW_C) = int_to_fp(int_sum, EEW_C, fmt_C, rm);
      acc = fp_add(acc, fp_mul(blk_scale, fp_sum, EEW_C, fmt_C, rm),
                   EEW_C, fmt_C, rm)
    };
...
if EEW_C == 32 & vtype[altfmt] == 0b1 then return Illegal_Instruction();
// SEW=64 is reserved
if EEW_C == 64 then return Illegal_Instruction();
...
// BS=16 LMUL restriction: W × LMUL ≤ SEW
if vtype[bs] == 0b1 & (4 * LMUL > EEW_C) then return Illegal_Instruction();
// Microscaling legality: SEW × λ ≥ 16
if EEW_C * lambda < 16 then return Illegal_Instruction();
...
let K_eff : int = lambda * 4 * LMUL;
let N     : int = unsigned(vl) / lambda;
let M     : int = (LMUL * vlen / EEW_A) / K_eff;
let MUL_C : int = M / lambda;

if unsigned(vl) % lambda != 0 then return Illegal_Instruction();
if MUL_C not in {1,2,4,8,16} then return Illegal_Instruction();
...
`altfmt` is unused and `vm` must be 1 (`vm=0` is _reserved_) for all three instructions.
For `vwmmacc.vv` and `vqwmmacc.vv`, `vm=0` is separately defined as `vfwimmacc.vv` and
`vfqwimmacc.vv` (integer inputs, FP accumulator, E8M0 microscaling); see <<tbl-intmx-encoding-map>>.
...
When `vm=0`, `vwmmacc.vv` and `vqwmmacc.vv` are decoded as `vfwimmacc.vv`
and `vfqwimmacc.vv` respectively.  MXINT inputs are always signed;

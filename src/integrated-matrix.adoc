...
This section defines three *recommended, non-normative* profiles intended to guide
software developers and implementers toward useful and interoperable subsets of the
Zvvm family for major application domains.

These profiles are descriptive bundles of existing subextensions.
They do not define new architectural requirements, and implementations are free to
support other combinations of subextensions.

==== ZvvHPC

The `ZvvHPC` profile is a recommended bundle of Zvvm computational subextensions
...
The `ZvvHPC` profile is characterized by support for:
...
Typical floating-point subextensions in `ZvvHPC` include:
...
but low-precision OFP4/OFP8 and microscaling-oriented subextensions are not the primary focus of this profile.

==== ZvvAIML-FP

The `ZvvAIML-FP` profile is a recommended bundle of Zvvm computational subextensions
...
The `ZvvAIML-FP` profile is characterized by support for:
...
Typical floating-point subextensions in `ZvvAIML-FP` include:
...
Implementations targeting `ZvvAIML-FP` may also support OFP4-derived widening subextensions
...
The `ZvvAIML-INT` profile is a recommended bundle of Zvvm computational subextensions
...
The `ZvvAIML-INT` profile is characterized by support for:
...
Typical integer subextensions in `ZvvAIML-INT` include:
...
Implementations targeting `ZvvAIML-INT` may also support wider integer accumulation
subextensions such as `Zvvi8i64mm`, `Zvvi16i64mm`, and `Zvvi32i64mm`, depending on workload requirements.

NOTE: The `ZvvHPC`, `ZvvAIML-FP`, and `ZvvAIML-INT` profiles are not mutually exclusive.
A high-end implementation may choose to support subextensions from more than one profile.
